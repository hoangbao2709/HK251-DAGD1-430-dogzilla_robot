...
class GoalPoseViewTests(TestCase):
    @patch("control.views.ROSClient")
    def test_posts_direct_goal_pose(self, ros_client_cls):
        ros_client = ros_client_cls.return_value
        ros_client.set_goal_pose.return_value = {
            "success": True,
            "message": "OK",
            "goal": {
                "x": 1.25,
        ...
        response = self.client.post(
            "/control/api/robots/robot-a/slam/goal/",
            data=json.dumps(
                {
                    "x": 1.25,
                    "y": -0.5,
        ...
        self.assertEqual(response.status_code, 200)
        ros_client.set_goal_pose.assert_called_once_with(x=1.25, y=-0.5, yaw=0.75)
...
class SessionSummaryTests(TestCase):
    def test_counts_only_today_obstacle_events_for_robot(self):
        robot, _ = Robot.objects.update_or_create(
            id="robot-a",
            defaults={"name": "Robot A"},
        )
        other_robot, _ = Robot.objects.update_or_create(
            id="robot-b",
            defaults={"name": "Robot B"},
        )
